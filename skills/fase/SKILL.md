...
Use `fase` when you want a local control plane over installed coding-agent CLIs instead of talking to one vendor CLI directly.
...
fase history search --json --query "deployment rollback"
```
...
```bash
fase run --json --adapter codex --cwd /path/to/repo --prompt "Fix the failing tests."
```
...
```bash
fase work list --json                    # list all work items
fase work show <work-id>                 # show details + docs + attestations
fase work ready --json                   # list actionable work
fase work create --title "..." --objective "..." --kind implement
fase inbox "quick idea"                  # shorthand for work create --kind idea
fase work update <work-id> --message "Started implementation"
fase work complete <work-id> --message "Done"
fase work note-add <work-id> --type finding --text "..."
fase work private-note <work-id> --text "SSH creds..." --type credential  # gitignored DB
fase work doc-set --file docs/adr-001.md                                 # auto-creates work item from doc
fase work doc-set <work-id> --file docs/adr-001.md --title "ADR-001"     # attach doc to existing work item
fase work attest <work-id> --result passed --summary "Tests pass" --verifier-kind deterministic --method test
fase work claim <work-id> --claimant worker-a
fase work release <work-id> --claimant worker-a
fase work renew-lease <work-id> --claimant worker-a --lease 15m          # heartbeat
fase work children <work-id> --json
fase work discover <work-id> --title "..." --objective "..." --kind verify --rationale "..."
fase work proposal create --type add_edge --target <work-id> --rationale "..." --patch '{"edge_type":"blocks","source_work_id":"<id>"}'
fase work approve <work-id> --message "Approved"
fase work reject <work-id> --message "Needs rework"
fase work lock <work-id>                 # human lock (prevents agent claim)
fase work unlock <work-id>
fase reconcile --json                    # release orphaned work with expired leases
fase artifacts attach --work <work-id> --path ./report.md --kind report
```

Supervisor (autonomous dispatch loop):

```bash
fase supervisor --default-adapter codex --max-concurrent 1  # run forever, dispatch work
fase supervisor --dry-run --json                            # show what would dispatch
fase supervisor --cwd /path/to/repo                        # target a specific repo
```
...
work graph. Each repo gets its own `.fase/` directory (per-repo state isolation).

Split databases:
- `.fase/fase.db` — public work graph (tracked in git)
...
- do not create children just to offload thinking or explore vaguely. If scope may expand or verification is unclear, use `fase work proposal create`.
...
```bash
fase debrief --json --session <session-id> --reason "prepare a debugging summary"
```
...
```bash
fase transfer export --json --job <job-id> --reason "provider outage" --mode recovery
fase transfer run --json --transfer <transfer-id-or-path> --adapter gemini --cwd /path/to/repo
```
...
- Treat `fase` as machine-facing first. Use `--json` unless a human-readable summary is explicitly better.
- Treat `work` as the source of truth; use prompts only as compiled hydration views over work state.
- If the host gives you an isolated runtime via inherited `FASE_*` env vars or a runtime-specific wrapper on `PATH`, use bare `fase` so all graph mutations land in the same runtime.
...
- Use `status --wait` when you want `fase` to own the polling loop.
...
- Do not expect `fase` to perform vendor auth flows for you.
...
- Treat adapter-native history import as a future special case when the session was not created by `fase`.
...
- Before claiming a planning phase succeeded, verify the expected child work actually exists with `fase work children` or `fase work show`.
...
- Prefer explicit `fase work update`, `fase work note-add`, and `fase work complete` calls over burying progress in tool transcripts.
