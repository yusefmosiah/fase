...
// cogentBridge is the acyclic interface for Cogent service tools.
// Using only core and primitive types avoids the service→adapters import cycle.
type cogentBridge interface {
...
// RegisterCogentTools registers check record and checker-specific tools into the registry.
// svcAny must satisfy cogentBridge (i.e., be *service.Service). If it does not, tool
// registration is skipped silently so non-service sessions still work.
func RegisterCogentTools(registry *ToolRegistry, svcAny any) error {
	svc, ok := svcAny.(cogentBridge)
	if !ok {
		return nil
	}
	tools := []Tool{
...
func newCheckRecordCreateTool(svc cogentBridge) Tool {
	type args struct {
...
func newCheckRecordListTool(svc cogentBridge) Tool {
	type args struct {
		WorkID string `json:"work_id"`
...
func newCheckRecordShowTool(svc cogentBridge) Tool {
	type args struct {
