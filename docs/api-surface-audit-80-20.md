# Cogent API Surface Audit: The 20% That Does 80% of the Work

## Executive Summary

Cogent has **30 top-level CLI commands** with **90+ subcommands** and **55+ HTTP endpoints**. Analysis of actual usage across supervisors, workers, MCP tools, web UI, tests, and documentation reveals that **~15% of the surface handles ~85% of real traffic**.
...
### `cogent work` Subcommands (30 total)
...
### `cogent check` Subcommands (3 total)
...
|-------|------------|---------|
| `cogent mcp` | `stdio`, `http`, `proxy` | `stdio` + `proxy` **ESSENTIAL**, `http` RARELY |
| `cogent login` | `chatgpt`, `status` | RARELY |
| `cogent catalog` | `sync`, `show`, `probe` | RARELY |
| `cogent artifacts` | `list`, `attach`, `show` | SOMETIMES |
| `cogent history` | `search` | RARELY |
| `cogent bootstrap` | `inspect`, `create` | RARELY |
| `cogent transfer` | `export`, `run` | RARELY |
| `cogent supervisor` | `pause`, `resume`, `send` | SOMETIMES (human control) |
...
|-----------|-----------|--------------|----------------|
| `cogent work check` | `cogent check create` | Both submit check records | **Merge**: keep `cogent check create` as canonical, deprecate `cogent work check` |
| `cogent work items` endpoint | `cogent work list` endpoint | Legacy vs canonical list | **Remove** `/api/work/items` (web UI should use `/api/work/list`) |
| `cogent report` | `cogent work update --message` | Both post messages | Keep both — `report` goes through channel, `update` mutates state |
| `cogent supervisor send` | `cogent serve --auto` supervisor steering | Both send to supervisor | Keep `supervisor send` as human control surface |
| `cogent runtime` (hidden) | `cogent adapters` | Overlapping adapter info | **Merge** into `cogent adapters` |

### Commands with Zero/Minimal Callers

| Command | Why It Exists | Recommendation |
|---------|---------------|----------------|
| `cogent work projection` | Debug projections (checklist, status) | **Remove** — `work show` + `hydrate` cover this |
| `cogent work proposal` | Structural graph edits | **Keep** but hide — used by supervisor for child work |
| `cogent work edge` | DAG management | **Keep** but hide — supervisor uses internally |
| `cogent work verify` | Cryptographic audit | **Keep** — security feature |
| `cogent work doc-set` | Doc-work coupling | **Keep** — ADR-0002 compliance |
| `cogent work promote` | Deployment promotion | **Keep** — future CI/CD integration |
| `cogent history search` | Full-text search | **Keep** but low priority |
| `cogent transfer` | Cross-vendor failover | **Keep** — recovery feature |
| `cogent login` | Auth flows | **Keep** — needed for ChatGPT |
| `cogent debrief` | Session self-assessment | **Keep** — recovery workflow |
| `cogent cancel` | Job cancellation | **Keep** — manual intervention |
| `cogent bootstrap` | Repo onboarding | **Keep** — initial setup |
...
```
cogent serve              # Start the service
cogent dispatch           # Dispatch ready work
cogent work create        # Create work items
cogent work show          # Inspect work state
cogent work list          # List work items
cogent work update        # Update work state (worker exit)
cogent work note-add      # Record findings
cogent work hydrate       # Generate briefings
cogent check create       # Submit check records
cogent check list         # Review checks
cogent report             # Notify supervisor/host
cogent mcp stdio/proxy    # Claude Code integration
...
1. **`cogent work projection`** subcommand group — `work show` + `hydrate` already render all projections
2. **`/api/work/items`** — legacy duplicate of `/api/work/list`; migrate web UI
3. **`/api/git/status`** — unused by web UI
4. **`cogent work check`** — merge into `cogent check create` (same endpoint, duplicate CLI path)

### Merge/Consolidate

1. **`cogent runtime`** → merge into **`cogent adapters`** (add `--runtime` flag)
2. **`cogent work check`** → use **`cogent check create`** only
3. **`/api/work/items`** → redirect to **`/api/work/list`**

### Hide (Keep but Mark Hidden)

1. `cogent __run-job` — already hidden, keep
2. `cogent work edge` — DAG internals, not user-facing
3. `cogent work proposal` — supervisor-internal

### Demote to Second Tier (Keep but De-prioritize)

1. `cogent catalog` — useful for initial setup only
2. `cogent history` — investigation tool, rarely needed
3. `cogent bootstrap` — one-time repo setup
4. `cogent login` — one-time auth
5. `cogent transfer` — recovery edge case
6. `cogent debrief` — recovery edge case
7. `cogent cancel` — manual intervention
...
| `cogent work` subcommands | 30 | 13 (43%) | 8 (27%) | 9 (30%) |
...
If Cogent were rebuilt from scratch supporting only the core workflow (create work → dispatch → work → check → attest → report), the CLI would be:

```
cogent serve                    # Run the service
cogent dispatch [work-id]       # Dispatch work
cogent work create              # Create work
cogent work show <id>           # Inspect work
cogent work list                # List work
cogent work update <id>         # Update state
cogent work note-add <id>       # Record findings
cogent check create <id>        # Submit check
cogent check list <id>          # Review checks
cogent report "<message>"       # Notify host
cogent mcp proxy                # Claude Code bridge
```
