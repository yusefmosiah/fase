...
1. **Orphan jobs.** Attestation jobs are created via `cogent run --work
...
3. **Invisible review contracts.** A work item's `required_attestations` JSON
   ...
   in the DAG. Tooling (UI, `cogent work show`, graph queries) cannot answer
...
The `CreateWork` service call (or the CLI `cogent work create` path) resolves
...
4. Record your finding:
   cogent work attest {{parent_work_id}} \
...
You MUST run exactly one cogent work attest command.
After attesting, mark this work item done:
   cogent work update {{self_work_id}} --state done
```

The attestor calls `cogent work attest` on the **parent** work ID (not its
...
calling `cogent work attest` rather than by an orphan job.
...
**Positive:**
- Attestation is visible in the work graph. `cogent work show <id>` shows
...
- ADR-0033: cogent serve (supervisor architecture, job monitoring loop)
