...
Today, agent identity in fase is implicit. The supervisor spawns workers and
attestors as `fase run` subprocesses, but there is no cryptographic binding
...
fase moves toward choir (distributed multi-VM orchestration), the trust
...
**fase (local):** The supervisor generates a long-lived Ed25519 CA keypair on
first run, stored at `$FASE_STATE_DIR/ca.key` (private) and `ca.pub`
(public). Each `fase run` invocation receives an ephemeral Ed25519 keypair
...
token (signed by VM intermediate CA). The `--trust-root` flag on `fase verify`
...
checks the token's capability list. A worker that attempts `fase work attest`
gets a hard rejection, not just a nonce mismatch.

**Token delivery:** The supervisor writes the token and agent private key to a
temporary file (mode 0600) and passes the path via the `FASE_AGENT_TOKEN`
...
worker subprocess (`Setpgid: true`) that survives the `fase run` launcher. The
...
same `FASE_AGENT_TOKEN` path (or a new path communicated via the lease renewal
...
```
GIT_COMMITTER_NAME=fase-worker-<job_id_short>
GIT_COMMITTER_EMAIL=<job_id>@fase.local
...
public keys, keyed by their fase email addresses. After a run completes, the
...
When an attestor calls `fase work attest`, the CLI:
...
├─ CA: fase-supervisor (ed25519:SHA256:abc...)
...
In the distributed case, `fase verify` walks a longer chain:
...
verify against. For local fase, this defaults to the supervisor CA. For choir,
...
- Operator can run `fase verify` in dry-run mode to see what would fail
- No behavior change for existing installs; enables observability before cutover
- Controlled by `FASE_CAPABILITY_ENFORCEMENT=audit|enforce` (default: `audit`)

**Phase 1: Capability tokens (CLI enforcement)**
- Generate supervisor CA keypair on first `fase supervisor` or `fase serve`
...
- Switch `FASE_CAPABILITY_ENFORCEMENT` default to `enforce`
...
- Ship minimal `fase verify <work-id>` covering certificate and commit signature checks
...
- Extend `fase verify` to check attestation signatures and nonce ordering

**Phase 4: Full verification tooling**
- Batch verification: `fase verify --all`
...
Existing fase installations have no CA keypair and no signed artifacts.

**Bootstrap:** On first `fase supervisor` or `fase serve` after upgrade:
...
3. `fase verify` on pre-Phase-1 work items returns `UNVERIFIED (legacy)` for
...
**CLI behavior during Phase 0:** Existing `fase work update` calls without a
...
clear error: `capability enforcement active: no token found (FASE_AGENT_TOKEN
not set)`. Human operators running commands interactively can obtain a short-
lived operator token via `fase token issue --role worker --work <work-id>`,
