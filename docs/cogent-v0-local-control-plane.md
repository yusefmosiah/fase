...
Status: Draft v0
Priority: 1
Requires: [docs/cogent-work-runtime.md, docs/cogent-work-api-and-schema.md, docs/cogent-worker-briefing-schema.md]
Owner: Runtime / Local Control Plane

## Narrative Summary (1-minute read)

`cogent` v0 should become a local work-control plane with a web UI.
...
`cogent` v0 is a local runtime for governed agent work.
...
Low-friction capture is essential. The inbox pattern should be:
    ...
    cogent inbox "thing I just thought of"
    cogent inbox "bug: X breaks when Y" --kind bug
...
- `cogent work show <id>` — work item with neighborhood
- `cogent logs <job-id>` — session logs for a specific job
- `cogent session <session-id>` — session detail and job list
- `cogent artifacts list --work <id>` — artifacts linked to work
- `cogent history search` — cross-reference with canonical history
...
### 12.5 CLI
    ...
    cogent work hydrate <work-id>                    # cold, standard density
    cogent work hydrate <work-id> --debrief          # resume original agent
    cogent work hydrate <work-id> --mode deep        # more evidence
    cogent work hydrate <work-id> --mode thin        # minimal, fast

## 14) Git Integration

Git already provides the content-history ledger.
`cogent` should integrate with Git, not reimplement it.

At minimum, `cogent` records should be able to point at:
- `base_commit_oid`
- `head_commit_oid`
- `branch_ref`
- `worktree_path`
...
Projection commands:
- `cogent work projection <work-id> --format md` — renders a work item as
  markdown (objective, acceptance, notes, attestations, history)
- `cogent project atlas` — renders an index of all work from the graph
- `cogent work hydrate <work-id>` — compiles a worker briefing as JSON
...
4. **Startup reconciliation**: on restart, `cogent` inspects store state,
...
`cogent` supervises work, leases, attestations, and approvals.
`cogent` does not supervise machines, VMs, or OS processes.
