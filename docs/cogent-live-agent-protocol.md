...
Status: Draft
Priority: 1
Requires: [docs/cogent-work-runtime.md, docs/host-agents.md]
Owner: Runtime / Agent Adapters

## Summary

`cogent` standardizes a **live agent protocol** for multi-agent orchestration
...
`cogent` is not normalizing every possible coding-agent CLI.
...
- replace `cogent`'s canonical job/session/work graph,
...
Each adapter has two independent concerns: **control transport** (how cogent
drives the agent) and **tool bridge** (how the agent calls cogent mid-turn).

```
                    Control Transport          Tool Bridge
                    (cogent → agent)          (agent → cogent)
                    ─────────────────         ────────────────
...
active. Events are the runtime observation surface for `cogent`.

Internally, `cogent` publishes events via the service-level `EventBus`.
...
- Codex: `turn/steer` with cogent messages
...
A tool call is an outbound invocation from the running agent into `cogent`.
...
The adapter MUST expose a stable native session identifier that `cogent` can
...
The adapter MUST allow `cogent` to inject a message into the currently active
...
The running agent MUST be able to call `cogent` tools during execution via
...
```text
[cogent:message from="agent-id" type="info|result|request"]
Message body.
[/cogent:message]
```
...
```text
[cogent:request from="agent-id" request_id="req_123"]
Question body.
[/cogent:request]

[cogent:response to="agent-id" request_id="req_123"]
Answer body.
[/cogent:response]
```
...
`cogent` remains authoritative for:
...
- Map Codex thread → cogent session, Codex turn → cogent turn

Key questions to answer:
- Does Codex app-server reliably support MCP server connections?
- How does Codex approval flow interact with cogent attestation?
...
- How does Pi's session file model map to cogent sessions?
...
4. **Session persistence**: How does each adapter's session model map to
   cogent's canonical session? Where are the impedance mismatches?
